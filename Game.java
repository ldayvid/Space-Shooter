...
class GamePanel extends JPanel implements KeyListener{
	...
	public void paintComponent(Graphics g) {
		...
		try {
			...
			background = new ImageIcon("src\\Images\\Level" + level + " Background.png");
				...
				// If bullets hit an enemy, remove the bullet and enemy loses one health
					...
					for(int j = 0; j < enemies.size(); j++) {
						if(enemies.get(j).intersects(pBullets.get(i))){
							enemies.get(j).loseHealth(pBullets.get(i).getDamage());
							pBullets.remove(i);
			...
			for(int i = 0; i < eBullets.size(); i++){
				...
				if(player.intersects(eBullets.get(i))){
					...
					if(!cheat){
						player.loseHealth(eBullets.get(i).getDamage());
					}
					eBullets.remove(i);
				...
				// If bullets hit an enemy, remove the bullet and enemy loses ten health
					...
					for(int j = 0; j < enemies.size(); j++) {
						if(enemies.get(j).intersects(bigBullets.get(i))){
							enemies.get(j).loseHealth(bigBullets.get(i).getDamage());
							bigBullets.remove(i);
				...
				if(player.intersects(enemies.get(i).detection())) {
					...
					if(enemies.get(i).getType()==1){
						...
						if(enemies.get(i).intersects(player)){
							if(!cheat){
								player.loseHealth(enemies.get(i).getDamage());
							}
	...
	public void loadLevel(){
		try {
			...
			// Read level file
			BufferedReader br = new BufferedReader(new FileReader(new File("src\\Maps\\Level"+level)));
			while((str = br.readLine() ) != null){
				...
				for(int j = 0; j < row.length; j++) {
					...
					// Create a rusher instance if file reads "3" in specified x and y
						...
						enemies.add(new Enemy(j*50,i*50,50, 50,2,1));
					}
